// gRPC test with JWT auth using @solsdk/jito-ts auth module
const grpc = require('@grpc/grpc-js')
const { SearcherServiceClient, SendBundleRequest } = require('@solsdk/jito-ts/dist/gen/block-engine/searcher')
const { AuthServiceClient } = require('@solsdk/jito-ts/dist/gen/block-engine/auth')
const { AuthProvider, authInterceptor } = require('@solsdk/jito-ts/dist/sdk/block-engine/auth')
...
async function main() {
  ...
  // Create gRPC client WITH JWT auth
  console.log('\n=== gRPC with JWT Auth ===')
  console.log('URL:', BLOCK_ENGINE)

  const authProvider = new AuthProvider(
    new AuthServiceClient(BLOCK_ENGINE, grpc.credentials.createSsl()),
    wallet,
  )
  ...
  console.log('Tip accounts count:', tipResp.accounts.length)
  ...
  console.log('\nSending bundle via gRPC with JWT auth...')
